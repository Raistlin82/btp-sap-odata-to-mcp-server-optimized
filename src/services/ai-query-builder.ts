...
import { QueryIntent, OptimizedQuery, FilterCondition, OrderByCondition } from '../types/ai-types.js';
...
export class AIQueryBuilderService {
    ...
    async buildQueryFromNaturalLanguage(
        ...
        try {
            ...
            const optimizedQuery = await this.optimizeQueryWithAI(
                interpretedIntent,
                entityType,
                context
            );
    ...
    private initializeQueryPatterns(): void {
        ...
        this.queryPatterns.set('high_value', {
            pattern: /^(show|find|get)\s+.*\b(high|large|big|expensive|costly|over|above|greater than)\b.*\b(\d+|amount|value|price|cost)\b/i,
            intent: {
                operation: 'filter',
                fields: [],
    ...
    private adaptTemplateToEntity(
        ...
        const fields = templateIntent.fields?.length ? templateIntent.fields : 
        ...
        if (templateIntent.filters) {
            for (const filter of templateIntent.filters) {
                if (filter.field === 'auto-detect') {
                    ...
                    // Find date field
                    const dateField = this.findFieldByPattern(entityType, /date|time|created|modified|updated/i);
                    if (dateField) {
                        filters.push({
                            ...filter,
    ...
    private buildIntentParsingPrompt(query: string, entityType: EntityType, context?: QueryContext): string {
...
Return structured intent with operation, fields, filters, and sorting.`;
    ...
    private buildOptimizationPrompt(intent: QueryIntent, entityType: EntityType, context?: QueryContext): string {
...
Focus on: field selection, filter efficiency, result size management.`;
    ...
    private parseAIResponseToIntent(aiResponse: OptimizedQuery, entityType: EntityType, query: string): QueryIntent {
    ...
    private applyOptimizationRules(baseUrl: string, intent: QueryIntent, entityType: EntityType): string {
    ...
    private determineCacheStrategy(intent: QueryIntent, entityType: EntityType): OptimizedQuery['cacheStrategy'] {
    ...
    private createFallbackQuery(query: string, entityType: EntityType, context?: QueryContext): NaturalQueryResult {
    ...
    private selectMostImportantFields(fields: string[], entityType: EntityType, limit: number): string[] {
    ...
    private suggestRelatedFields(currentFields: string[], entityType: EntityType): string[] {
        ...
        if (currentFields.includes('Amount') || currentFields.some(f => /amount|value|price/i.test(f))) {
            suggestions.push(...entityType.properties
    ...
    private extractStatusFromQuery(query: string): string {
        const statusMatch = query.match(/\b(pending|active|completed|approved|rejected|open|closed)\b/i);
    ...
    private buildQueryUrl(entitySet: string, options: {
        select?: string[];
    ...
    private optimizeFilterOrder(url: string, filters: FilterCondition[], entityType: EntityType): string {
