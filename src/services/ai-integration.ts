...
export class AIIntegrationService {
    ...
    async analyzeData(
        ...
        try {
            ...
            this.logger.error('AI analysis failed', { error: error instanceof Error ? error.message : 'Unknown error' });
            return this.createFallbackAnalysis(data, analysisType);
        }
    }
    ...
    async optimizeQuery(
        ...
        try {
            ...
            this.logger.error('Query optimization failed', { error: error instanceof Error ? error.message : 'Unknown error' });
            return this.createBasicQuery(userIntent, entityMetadata);
        }
    }
    ...
    async detectAnomalies(
        ...
        try {
            ...
            this.logger.error('Anomaly detection failed', { error: error instanceof Error ? error.message : 'Unknown error' });
            return this.createBasicAnomalyDetection(currentData, historicalData, entitySet);
        }
    }
    ...
    async generateBusinessInsights(
        ...
        try {
            const insightsPrompt = this.buildBusinessInsightsPrompt(
                data, 
                entitySet, 
                businessContext
            );
            ...
            this.logger.error('Business insights generation failed', { error: error instanceof Error ? error.message : 'Unknown error' });
            return this.createBasicInsights(data, entitySet);
        }
    }
...
Generate business-relevant insights from this SAP data.
    ...
    private parseQueryOptimizationResponse(
        ...
        return {
            url: `${entityMetadata.entitySet}?$select=${entityMetadata.properties.slice(0, 5).map(p => p.name).join(',')}`,
    ...
    private createBasicAnomalyDetection(
        ...
        const volumeAnomaly = Math.abs(currentData.length - historicalData.length) > (historicalData.length * 0.5);
