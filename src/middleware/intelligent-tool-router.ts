...
export class IntelligentToolRouter {
   ...
   */
  public analyzeRequest(userRequest: string, _context?: any): RoutingResult {
    ...
    const uiItalianMatch = this.checkUIPatterns(
      request,
      config.toolSelectionRules.uiPatterns?.italian || []
    );
    const uiEnglishMatch = this.checkUIPatterns(
      request,
      config.toolSelectionRules.uiPatterns?.english || []
    );
    ...
    const directQueryMatch = this.checkDirectQueryPatterns(
      request,
      config.toolSelectionRules.directQueryPatterns
    );
    if (directQueryMatch.matched) {
      return {
        ...
        suggestedSequence: config.workflowSequences.directExecution?.map(
          (step: any) => step.tool
    ...
    const performanceMatch = this.checkPatterns(
      request,
      config.toolSelectionRules.performancePatterns
    );
    ...
    if (processMatch.matched) {
      return {
        ...
        suggestedSequence: config.workflowSequences.businessProcessAnalysis?.map(
          (step: any) => step.tool
        ) || ['execute-entity-operation', 'business-process-insights'],
      };
    }

    // 5. Check for natural language patterns (Italian/English)
    const italianMatch = this.checkPatternsWithTool(
      request,
      config.toolSelectionRules.naturalLanguagePatterns.italian
    );
    const englishMatch = this.checkPatternsWithTool(
      request,
      config.toolSelectionRules.naturalLanguagePatterns.english
    );
   ...
   */
  public getSuggestedWorkflow(
    routingResult: RoutingResult,
    includeDiscovery: boolean = true
  ): string[] {
    const _config = this.workflowConfig.loadConfig();

    // Add discovery phase if needed
    const discoveryPhase = includeDiscovery
      ? ['search-sap-services', 'discover-service-entities', 'get-entity-schema']
      : [];

    // Return discovery + suggested sequence
    return [
      ...discoveryPhase,
      ...(routingResult.suggestedSequence || [routingResult.selectedTool]),
    ];
  }
   ...
   */
  private checkDirectQueryPatterns(
    request: string,
    patterns: string[]
   ...
   */
  public validateWorkflowSequence(
    currentTool: string,
   ...
   */
  private suggestUIToolForWorkflow(
    currentTool: string,
    previousTools: string[],
    userRequest: string
  ): string | undefined {
    try {
      ...
      if (
        ...
        request.includes('edit') ||
        request.includes('modifica')
      ) {
      ...
      if (
        ...
        request.includes('grid') ||
        request.includes('list') ||
        request.includes('tabella') ||
        request.includes('elenco')
      ) {
        return 'ui-data-grid';
      }
      ...
      if (
        ...
        request.includes('kpi') ||
        request.includes('metric') ||
        request.includes('analytic')
      ) {
      ...
      if (
        ...
        request.includes('drill') ||
        request.includes('analysis') ||
        request.includes('rapporto')
      ) {
        return 'ui-report-builder';
      }

      if (
        request.includes('workflow') ||
        request.includes('process') ||
        request.includes('approval') ||
        request.includes('approvazione')
      ) {
      ...
      if (currentTool === 'execute-entity-operation') {
        ...
        if (
          previousTools.includes('discover-service-entities') ||
        ...
        if (
          request.includes('create') ||
          request.includes('update') ||
          request.includes('insert')
        ) {
          return 'ui-form-generator';
        }
      }
