...
import {
  NaturalQueryBuilderTool,
  SmartDataAnalysisTool,
  QueryPerformanceOptimizerTool,
  BusinessProcessInsightsTool,
} from './ai-enhanced-tools.js';
...
export class HierarchicalSAPToolRegistry {
  ...
  constructor(
    ...
    if (tokenStore && authServerUrl) {
      ...
      this.logger.warn(
        `⚠️  MCPAuthManager NOT initialized - tokenStore: ${!!tokenStore}, authServerUrl: ${authServerUrl}`
      );
    }
  }
  ...
  public async updateDiscoveredServices(newDiscoveredServices: ODataService[]): Promise<void> {
    this.logger.info(
      `🔄 Updating discovered services from ${this.discoveredServices.length} to ${newDiscoveredServices.length} services`
    );
    ...
    this.logger.info(
      `✅ Discovered services updated successfully. sap://services resource will now reflect filtered services.`
    );
  }
  ...
  public async registerDiscoveryTools(): Promise<void> {
    this.logger.info(
      `🔧 Registering hierarchical tools for ${this.discoveredServices.length} services`
    );
      ...
      'search-sap-services',
      {
        ...
        inputSchema: {
          query: z
            .string()
            .optional()
            .describe('Search term to filter services (name, title, description)'),
          category: z
            .enum(['business-partner', 'sales', 'finance', 'procurement', 'hr', 'logistics', 'all'])
            .optional()
            .describe('Service category filter'),
          limit: z
            .number()
            .min(1)
            .max(20)
      ...
      'discover-service-entities',
      {
        ...
        inputSchema: {
          ...
          showCapabilities: z
            .boolean()
            .default(true)
            .describe('Show CRUD capabilities for each entity'),
        },
      },
      async (args: Record<string, unknown>) => {
    ...
    this.mcpServer.registerTool(
      'execute-entity-operation',
      {
        ...
        inputSchema: {
          ...
          operation: z
            .enum(['read', 'read-single', 'create', 'update', 'delete'])
            .describe('The operation to perform'),
          parameters: z
            .record(z.any())
            .optional()
            .describe('Operation parameters (keys, filters, data, etc.)'),
          queryOptions: z
            .object({
            ...
            .optional()
            .describe('OData query options (for read operations)'),
        },
      },
      async (args: Record<string, unknown>) => {
      ...
      'check-sap-authentication',
      {
        ...
        inputSchema: {
          session_id: z
            ...
            .optional()
            .describe(
              'User session ID obtained from OAuth authentication. Provide this to authenticate and associate with your MCP session.'
            ),
          validateSession: z
            .boolean()
            .default(true)
            .describe('Whether to validate existing session'),
          requestPreAuth: z
            .boolean()
            .default(true)
            .describe(
              'Whether to request pre-authentication for upcoming operations (recommended: true)'
            ),
          context: z
            .object({
              anticipatedOperations: z
                .array(z.enum(['read', 'create', 'update', 'delete', 'analysis']))
                .optional()
                .describe('Operations user plans to perform'),
              sessionType: z
                .enum(['interactive', 'batch', 'demo'])
                .optional()
                .describe('Type of session being initiated'),
            })
            .optional()
            .describe('Context for authentication check'),
        },
      },
      async (args: Record<string, unknown>) => {
        try {
          ...
          this.logger.info(
            `🔐 Proactive auth check - sessionId provided: ${!!sessionId}, validate: ${validateSession}, preAuth: ${requestPreAuth}`
          );
          ...
          if (sessionId && this.authManager) {
            ...
            try {
              ...
              const authResult = await this.authManager.authenticateToolCall(
                'execute-entity-operation',
              ...
              if (authResult.authenticated) {
                this.logger.info(
                  `✅ Session ID authentication successful for user: ${authResult.context?.user}`
                );
                ...
                this.logger.warn(
                  `❌ Session ID authentication failed: ${authResult.error?.message}`
                );
                      ...
                      text: JSON.stringify(
                        {
                          ...
                          action:
                            'Visit auth_url, get new session_id, call check-sap-authentication again',
                        },
                        null,
                        2
                      ),
          ...
          try {
            ...
            const connectivityTest = await this.authManager.authenticateToolCall(
              'connectivity-test',
              {}
            );
          ...
          if (authStatus.isAuthenticated && authStatus.sessionValid) {
            ...
            response.auth_url = this.authManager
              ? `${this.authManager.getAuthServerUrl()}/auth/`
              : 'Auth URL not available';
            response.action =
              'Visit auth_url → get session_id → call check-sap-authentication({session_id: "your_id"})';
            ...
            if (requestPreAuth) {
              ...
              try {
                ...
                const preAuthResult = await this.authManager.authenticateToolCall(
                  'check-sap-authentication',
                  {
                    preAuthRequest: true,
                    context: context,
                ...
                response.preAuthInstructions = {
                  message:
                    'Pre-authentication failed. You can authenticate later when using execution tools',
                  fallback:
                    'Authentication will be requested when needed during workflow execution',
                };
              }
            } else {
              response.preAuthInstructions = {
                ...
                text: JSON.stringify(
                  {
                    ...
                    fallback: {
                      recommendation:
                        'Proceed with discovery tools, authentication will be checked when needed',
                      safeTools: [
                        'search-sap-services',
                        'discover-service-entities',
                        'get-entity-schema',
      ...
      async (args: Record<string, unknown>) => {
        try {
          ...
          if (!context.previousTools?.includes('check-sap-authentication')) {
            this.logger.debug(
              '🔐 Note: Authentication check not in previous tools - should have been done at session start'
            );
          }
          ...
          this.logger.info(
            `🎯 Router selected: ${routingResult.selectedTool} (confidence: ${routingResult.confidence})`
          );
          ...
          if (routingResult.requiresAuth || routingResult.uiIntent) {
            ...
            try {
              ...
              if (!authResult.authenticated) {
                this.logger.warn(
                  `❌ Authentication required for UI tool: ${routingResult.uiIntent || routingResult.selectedTool}`
                );
              ...
              this.logger.info(
                `✅ Authentication validation passed for UI tool: ${routingResult.uiIntent}`
              );
            } catch (error) {
          ...
          const fullWorkflow = this.intelligentRouter.getSuggestedWorkflow(
            routingResult,
            !context.serviceId
          );
                ...
                text: JSON.stringify(
                  {
                    ...
                    fallback: {
                      recommendation:
                        'Use natural-query-builder for natural language or execute-entity-operation for direct OData queries',
                      workflow: [
                        'search-sap-services',
                        'discover-service-entities',
                        'natural-query-builder',
  ...
  private async registerAIEnhancedTools(): Promise<void> {
    ...
    this.mcpServer.registerTool(
      ...
      async (args: Record<string, unknown>) => {
        try {
          ...
          this.logger.debug(
            `🔄 Executing natural-query-builder (design-time transformation, no auth required)`
          );
      ...
      'smart-data-analysis',
      {
        ...
        inputSchema: {
          data: z
            ...
            .describe('Array of data records to analyze - each record is a key-value object'),
          analysisType: z
            .enum(['trend', 'anomaly', 'forecast', 'correlation'])
      ...
      async (args: Record<string, unknown>) => {
        try {
          ...
          if (this.authManager) {
            ...
            this.logger.warn(
              `⚠️  No authentication manager available - smart-data-analysis will proceed without authentication`
            );
          }
    ...
    this.mcpServer.registerTool(
      'query-performance-optimizer',
      {
        ...
        inputSchema: {
          ...
          optimizationGoals: z
            .array(z.enum(['speed', 'bandwidth', 'accuracy', 'caching']))
            .optional()
            .describe('Primary optimization objectives'),
        },
      },
      async (args: Record<string, unknown>) => {
        try {
          ...
          if (this.authManager) {
            this.logger.debug(
              `🔐 Authentication required for query-performance-optimizer, checking...`
            );
            const authResult = await this.authManager.authenticateToolCall(
              'query-performance-optimizer',
              args
            ...
            this.logger.warn(
              `⚠️  No authentication manager available - query-performance-optimizer will proceed without authentication`
            );
          }
    ...
    this.mcpServer.registerTool(
      'business-process-insights',
      {
        ...
        inputSchema: {
          processType: z
            .enum(['procurement', 'sales', 'finance', 'inventory', 'hr', 'general'])
            .describe('Type of business process to analyze'),
          ...
          focusAreas: z
            .array(z.enum(['efficiency', 'costs', 'compliance', 'quality', 'speed']))
            .optional()
      ...
      async (args: Record<string, unknown>) => {
        try {
          ...
          if (this.authManager) {
            this.logger.debug(
              `🔐 Authentication required for business-process-insights, checking...`
            );
            const authResult = await this.authManager.authenticateToolCall(
              'business-process-insights',
              args
            ...
            this.logger.warn(
              `⚠️  No authentication manager available - business-process-insights will proceed without authentication`
            );
          }
  ...
  private async registerRealtimeAnalyticsTools(): Promise<void> {
    ...
    this.logger.info(
      '✅ Registered 4 Real-time Analytics tools: realtime-data-stream, kpi-dashboard-builder, predictive-analytics-engine, business-intelligence-insights'
    );
  }
  ...
  private async searchServices(args: Record<string, unknown>) {
    try {
      ...
      this.logger.info(
        `🔍 Searching local services: query="${query}", category="${category}", limit=${limit}`
      );
  ...
  private categorizeServices(): void {
    for (const service of this.discoveredServices) {
      ...
      if (this.processClassification) {
        ...
        for (const [categoryKey, category] of Object.entries(
          this.processClassification.processCategories
        )) {
        ...
        for (const [categoryKey, category] of Object.entries(
          this.processClassification.crossFunctionalProcesses
        )) {
          if (this.matchesKeywords(searchText, category.keywords)) {
            categories.push(categoryKey);
          }
        ...
        for (const [categoryKey, category] of Object.entries(
          this.processClassification.industrySpecific
        )) {
          if (this.matchesKeywords(searchText, category.keywords)) {
            categories.push(categoryKey);
          }
    ...
    this.logger.debug(
      `Categorized ${this.discoveredServices.length} services into categories using ${this.processClassification ? 'SAP Signavio' : 'basic'} classification`
    );
  }
    ...
    try {
      ...
      if (!entityType) {
        const availableEntities =
          service.metadata?.entityTypes?.map(e => e.name).join(', ') || 'none';
        return {
          content: [
            {
              type: 'text' as const,
    ...
    try {
      ...
      if (this.authManager) {
        ...
        if (authResult.context?.token) {
          ...
          this.logger.debug(
            `User authenticated - no JWT token available (will use BasicAuth if configured in destination)`
          );
        }
      } else {
        this.logger.warn(
          `⚠️  No authentication manager available - execute-entity-operation will proceed without authentication`
        );
      }
      ...
      switch (operation) {
        ...
        case 'read-single':
          ...
          response = await this.sapClient.executeCRUDOperation(
            'read',
            singleReadUrl,
            undefined,
            userJWT
        ...
        case 'delete':
          ...
          response = {
            data: {
              message: `Successfully deleted ${entityName} with key: ${deleteKeyValue}`,
              success: true,
            },
          };
          break;
      ...
      const uiSuggestions = this.generateUIToolSuggestions(
        operation,
  ...
  private buildKeyValue(entityType: EntityType, parameters: Record<string, any>): string {
    ...
    if (keyProperties.length === 1) {
      ...
      if (!(keyName in parameters)) {
        throw new Error(
          `Missing required key property: ${keyName}. Required keys: ${entityType.keys.join(', ')}`
        );
      }
    ...
    const keyParts = keyProperties.map(prop => {
      if (!(prop.name in parameters)) {
        throw new Error(
          `Missing required key property: ${prop.name}. Required keys: ${entityType.keys.join(', ')}`
        );
      }
  ...
  public registerServiceMetadataResources(): void {
    ...
    this.logger.info(
      '✅ MCP Document Grounding Resources registered: sap://routing-rules, sap://service/{id}/metadata, sap://services'
    );
  }
  ...
  private async registerUITools(): Promise<void> {
    try {
        ...
        'ui-form-generator',
        {
          ...
          inputSchema: {
            ...
            fields: z
              ...
              .optional()
              .describe('Custom form fields configuration'),
          },
        },
        async (args: Record<string, unknown>) => {
          try {
        ...
        'ui-data-grid',
        {
          ...
          inputSchema: {
            ...
            features: z
              ...
              .optional()
              .describe('Grid feature enablement'),
          },
        },
        async (args: Record<string, unknown>) => {
          try {
      ...
      this.mcpServer.registerTool(
        'ui-report-builder',
        {
          ...
          inputSchema: {
            ...
            reportType: z
              .enum(['summary', 'detailed', 'analytical', 'custom'])
      ...
      this.logger.info(
        '✅ All UI Tools registered successfully: ui-form-generator, ui-data-grid, ui-dashboard-composer, ui-workflow-builder, ui-report-builder'
      );
    } catch (error) {
  ...
  private generateFormHTML(entityType: string, formType: string, fields?: any[]): string {
    ...
    const fieldsHtml = defaultFields
      .map(
        field => `
...
<body>
  ...
  private generateDataGridHTML(entityType: string, columns: any[], features: any): string {
    ...
    const headersHtml = defaultColumns
      .map(col => `<th onclick="sortTable('${col.key}')">${col.label} ↕️</th>`)
      .join('');
  ...
  private generateDashboardHTML(title: string, widgets: any[], layout: string): string {
    const widgetsHtml =
      widgets
        ...
        .join('') ||
      '<div class="widget"><div class="widget-title">Sample KPI</div><div class="metric-value">42</div></div>';
  ...
  private generateWorkflowHTML(workflowName: string, steps: any[], entityType: string): string {
    const stepsHtml =
      steps
        ?.map((step, index) => {
          const connector =
            index < steps.length - 1 ? '<div class="workflow-connector">↓</div>' : '';
        ...
        .join('') ||
      '<div class="step"><div class="step-header"><span class="step-type">form</span>Default Step</div><p>Sample workflow step</p></div>';
  ...
  private generateReportHTML(
    ...
    const metricsHtml =
      measures
        ...
        .join('') ||
      '<div class="metric-card"><div class="metric-value">42</div><div class="metric-label">Sample Metric</div></div>';
...
<body>
   ...
   */
  private generateUIToolSuggestions(
    operation: string,
    entityName: string,
    responseData: any
  ): string | null {
    try {
      ...
      switch (operation) {
        case 'read':
          ...
          const isMultipleRecords =
            Array.isArray(responseData?.value) || Array.isArray(responseData?.d?.results);
  ...
  private generateEntityDiscoveryUIToolSuggestions(entityName: string, schema: any): string | null {
    try {
      ...
      const hasNumericFields = schema.properties?.some(
        (prop: any) =>
          prop.type?.includes('Int') ||
          prop.type?.includes('Decimal') ||
          prop.type?.includes('Double')
      );
      ...
      const hasWorkflowFields = schema.properties?.some(
        (prop: any) =>
