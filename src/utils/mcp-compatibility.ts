...
export class MCPCompatibilityManager {
    ...
    private constructor() {
        ...
        this.options = {
            ...
            maxResponseLength: process.env.MCP_MAX_RESPONSE_LENGTH ?
                parseInt(process.env.MCP_MAX_RESPONSE_LENGTH) : undefined,
            legacyClients
        };
    }
    ...
    public isFullFeatureClient(clientId?: string): boolean {
        ...
        const isLegacyClient = this.options.legacyClients?.some(pattern =>
...
export function wrapToolResponse(response: any, toolName: string, clientId?: string): any {
    ...
    const shouldWrapResponse = manager.getSettings().simplifiedResponses ||
        !manager.isFullFeatureClient(clientId);
